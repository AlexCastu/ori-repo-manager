...
import {
  ...
  AlertCircle, Upload, Check as CheckIcon
} from 'lucide-react';
...
export function ProjectGrid() {
  ...
  // Contadores de estado git (sobre proyectos visibles)
  const statusCounts = useMemo(() => {
    let conCambios = 0;
    let paraPush = 0;
    let alDia = 0;
    for (const p of filteredProjects) {
      const s = gitStatuses[p.path];
      if (!s) continue;
      if (s.has_changes) conCambios++;
      if (s.ahead > 0) paraPush++;
      if (!s.has_changes && s.ahead === 0 && s.behind === 0) alDia++;
    }
    return { conCambios, paraPush, alDia };
  }, [filteredProjects, gitStatuses]);

  // Número de filtros activos
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (filters.gitStatus !== 'all') count++;
    if (filters.platforms.length > 0) count++;
    if (filters.branches.length > 0) count++;
    if (filters.tags.length > 0) count++;
    if (filters.hasUncommitted !== null) count++;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              ...
              {activeFilterCount > 0 && (
                <span className="flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold text-white bg-[var(--primary)]">
                  {activeFilterCount}
                </span>
              )}
            </button>
      ...
      {(filteredProjects.length > 0 || hiddenCount > 0) && (
        <div
          className="px-4 py-2 border-t border-[var(--border)] flex items-center justify-between gap-3"
          style={{
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {/* Status counters */}
            {(statusCounts.conCambios > 0 || statusCounts.paraPush > 0 || statusCounts.alDia > 0) && (
              <div className="flex items-center gap-2 text-[11px]">
                <span className="w-px h-3 bg-[var(--border)]" />
                {statusCounts.conCambios > 0 && (
                  <span className="flex items-center gap-1" style={{ color: 'var(--warning-text)' }}>
                    <AlertCircle className="w-3 h-3" />
                    {statusCounts.conCambios}
                  </span>
                )}
                {statusCounts.paraPush > 0 && (
                  <span className="flex items-center gap-1" style={{ color: 'var(--primary)' }}>
                    <Upload className="w-3 h-3" />
                    {statusCounts.paraPush}
                  </span>
                )}
                {statusCounts.alDia > 0 && (
